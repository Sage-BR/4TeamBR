...
public class QueenAnt extends Quest implements Runnable
{
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		switch (event)
		{
			...
			{
				for (L2MonsterInstance mob : royalGuards)
				{
					if (mob != null)
					{
						mob.decayMe();
				...
				for (final L2MonsterInstance nurse : antNurses)
				{
					if (nurse != null)
					{
						nurse.decayMe();
			...
			case "HEAL":
			{
				...
				for (L2MonsterInstance antNurse : antNurses)
				{
					...
					if (nurseNeedHeal)
					{
						...
						{
							for (L2MonsterInstance element : antNurses)
							{
								getIntoPosition(element, antNurse);
								element.setTarget(antNurse);
