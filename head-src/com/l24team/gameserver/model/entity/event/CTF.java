...
public class CTF implements EventTask
{
	...
	{
		if (player == null || !player.isOnline() || !(player.getX() > eventCenterX - eventOffset && player.getX() < eventCenterX + eventOffset && player.getY() > eventCenterY - eventOffset && player.getY() < eventCenterY + eventOffset && player.getZ() > eventCenterZ - eventOffset && player.getZ() < eventCenterZ + eventOffset))
		{
			return true;
		}
		return false;
